...
def initialize_datasets(args, datadir, dataset, subset=None, splits=None,
                        ...
                        remove_h=False, finetune=False, basic_prob=False):
    """
    Initialize datasets.
    ...
    if dataset != 'qm9':
        ...
        if dataset == 'qm9_second_half' or dataset == 'qm9_second_half2':
    ...
    if finetune: # same with pcq
        all_species = torch.tensor([1, 2, 4, 5, 6, 7, 8, 9, 12, 14, 15, 16, 17, 18, 20, 22, 30, 31, 32, 33, 34, 35])

    # Now initialize MolecularDataset based upon loaded data
    datasets = {split: ProcessedDataset(data, num_pts=num_pts.get(
        split, -1), included_species=all_species, subtract_thermo=subtract_thermo, basic_prob=basic_prob) for split, data in datasets.items()}
