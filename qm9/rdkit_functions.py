...
def compute_qm9_smiles(dataset_name, remove_h):
    ...
    :return:
    '''
    print("\tConverting QM9_ dataset to SMILES ...")
    ...
    class StaticArgs:
        def __init__(self, dataset, remove_h):
            ...
            if dataset_name == 'qm9_second_half2':
                self.finetune = True
    ...
    dataset_info = get_dataset_info(args_dataset.dataset, args_dataset.remove_h, finetune)
    n_types = 4 if remove_h else 5
    if args_dataset.dataset == 'qm9_second_half2':
        n_types = 22
    mols_smiles = []
    ...
    print("mols_smiles: ", len(mols_smiles))
    return mols_smiles
...
def retrieve_qm9_smiles(dataset_info):
    ...
    try:
        ...
        print("smile file path: %s" % file_name)
...
def build_xae_molecule(positions, atom_types, dataset_info):
    ...
    for i in range(n):
        for j in range(i):
            ...
            elif dataset_info['name'] in ['geom', 'pcq']:
