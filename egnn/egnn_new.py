...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, device='cpu', act_fn=nn.SiLU(), n_layers=3, attention=False,
                 ...
                 sin_embedding=False, normalization_factor=100, aggregation_method='sum', condition_decoupling=False, context_basis_dim=0, branch_layers_num=2, condition=False, later_fusion=False):
        ...
        if later_fusion:
            self.embedding2 = nn.Linear(in_node_nf - 2, self.hidden_nf) # in_node_nf includes time and charge, only use one hot encoding for atom type
            self.atomtype_add_layer = EquivariantBlock(hidden_nf, edge_feat_nf=edge_feat_nf, device=device,
                                                               act_fn=act_fn, n_layers=inv_sublayers,
                                                               attention=attention, norm_diff=norm_diff, tanh=tanh,
                                                               coords_range=coords_range, norm_constant=norm_constant,
                                                               sin_embedding=self.sin_embedding,
        ...
        self.T = 1000
        self.prediction_threshold_t = 10
