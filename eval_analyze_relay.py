...
def main():
    ...
    parser.add_argument("--relay_sampling", type=int, default=0, help="Whether to use relay sampling or not.")
    parser.add_argument("--second_model_path", type=str, default=None, help="Path to second model for relay sampling.")
    
    eval_args, unparsed_args = parser.parse_known_args()

    assert eval_args.model_path is not None
    assert eval_args.second_model_path is not None or eval_args.relay_sampling == 0, "Please specify second model path for relay sampling."

    with open(join(eval_args.model_path, 'args.pickle'), 'rb') as f:
        args = pickle.load(f)
    with open(join(eval_args.second_model_path, 'args.pickle'), 'rb') as f:
        second_args = pickle.load(f)
    ...
    second_generative_model, _, _ = get_model(second_args, device, dataset_info, dataloaders['train'])
    if prop_dist is not None:
        property_norms = compute_mean_mad(dataloaders, args.conditioning, args.dataset)
        prop_dist.set_normalizer(property_norms)
    generative_model.to(device)
    second_generative_model.to(device)
    ...
    fn = 'generative_model_ema.npy' if args.ema_decay > 0 else 'generative_model.npy'
    print(f"Loading {fn} from {eval_args.second_model_path}")
    second_flow_state_dict = torch.load(join(eval_args.second_model_path, fn), map_location=device)
    second_generative_model.load_state_dict(second_flow_state_dict)
    
    
    
    generative_model.second_dynamics = second_generative_model.dynamics
